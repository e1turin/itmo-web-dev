...
- [x] Интерактивные часы, показывающие текущие дату и время, обновляющиеся раз в __8 секунд__.
...
- [x] Набор компонентов для задания координат точки и радиуса области в соответствии с вариантом задания. Может
  потребоваться использование дополнительных библиотек компонентов - __ICEfaces__ (префикс "`ace`") и __PrimeFaces__ (
...
> __Минимальный__ рабочий пример см. в коммите [e9272c2](https://github.com/e1turin/itmo-webdev/tree/e9272c25507fecbc27451a2be349a176d7012568)

## Дополнительные требования к приложению:

- [x] Все результаты проверки должны сохраняться в базе данных под управлением СУБД **PostgreSQL**.
  - > Чтобы использовать мой пример, нужно создать базу данных подобно схеме (`main/resources/database/scheme.sql`) 
    > и указать переменные среды для доступа к БД (см. `Syste.getenv` в `model/database/DbManager.java`)
- [x] Для доступа к БД необходимо использовать протокол __JDBC без каких-либо дополнительных библиотек__.
- [x] Для управления списком результатов должен использоваться __Session-scoped Managed Bean__.
- [x] Конфигурация __управляемых__ бинов должна быть задана с помощью _аннотаций_.
